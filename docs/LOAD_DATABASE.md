...
| `gemma4:latest` (8B) | ~8.1 ГиБ | **Нестабильно** — свободная RAM на этой машине колеблется 3–8 ГБ в зависимости от фона (браузер и т.д.), 3 из 4 попыток провалились с `model requires more system memory`. `OLLAMA_CONTEXT_LENGTH` не спасает — требование определяется весами модели, не контекстом. Закрыть тяжёлые приложения перед запуском. |
